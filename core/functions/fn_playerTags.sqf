...
#define iconID 78000
#define scale 0.8
_headgear = ["H_Shemag_olive","H_Shemag_khk","H_ShemagOpen_tan","H_ShemagOpen_khk","H_Shemag_olive_hs","H_Shemag_tan"];
	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then {
		...
		if(count _sPos > 1 && {_distance < 15}) then {
			_text = switch (true) do {
				case ((headgear _x) in _headgear): {format["<t color='#FFFFFF'>Masked Player</t>"];};
				case ((goggles _x) in _goggles): {format["<t color='#FFFFFF'>Masked Player</t>"];};
